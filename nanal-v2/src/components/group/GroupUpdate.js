        ...
        if (data.statusCode === 200) {
          ...
          if (data.data.responseMessage === '그룹 조회 성공') {
            ...
            axios_api
              ...
              .catch(({ error }) => {
                console.log('그룹에 속하지 않은 친구 리스트 조회 오류: ' + error);
              });
          }
        } else {
          ...
          <div>
            <form onSubmit={GroupUpdate}>
              ...
              <div id='group-image' className='mb-2'>
                <p>💙 그룹 이미지 </p>
                <div className='flex'>
                  <img src={groupImg} className='inline-block w-24 h-24 p-1 mr-3 rounded-md'></img>
                  <p className='my-2'>
                    <input
                      type='file'
                      accept='image/*'
          ...
          <div id='group-Friend'>
            ...
            <div className='items-center justify-between text-center'>
              {groupNotFriendList.length >= 0 ? (
                ...
                <p className='my-2 text-center'>모든 친구가 이미 그룹에 속해있습니다.</p>
              )}
            </div>
          </div>
        </div>
